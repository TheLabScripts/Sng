...
const primaryTabs = [
  ...
  { label: "Scan", href: "/app/scan/", icon: ScanIcon, featured: true },
  { label: "Watchlist", href: "/app/watchlists/", icon: WatchIcon },
] as const;

const moreItems = [
  { label: "Analyze Listing", href: "/app/analyze/", icon: AnalyzeIcon },
  { label: "Field Scan", href: "/app/field-scan/", icon: ScanIcon },
  { label: "VIN Scan", href: "/app/vehicle-mode/", icon: CarIcon },
  { label: "Similar Sales", href: "/app/analyze/", icon: CompsIcon },
          ...
          <header className="sticky top-0 z-40 bg-bg/76 px-4 pb-2 pt-4 backdrop-blur sm:px-6">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <Link href="/app/scan/" className="grid h-10 w-10 place-items-center rounded-card border border-brand/35 bg-brand/10 text-brand" aria-label="Open scan tools"><ScanIcon /></Link>
                <ThemeToggle compact />
                <button type="button" onClick={signOut} className="hidden h-10 rounded-card border border-line bg-surface px-3 text-sm text-muted transition hover:text-ink sm:inline-flex sm:items-center">Sign out</button>
              </div>
            </div>
          </header>
...
function MoreSheet({ pathname, items, onClose }: { pathname: string; items: readonly { label: string; href: string; icon: () => React.ReactNode }[]; onClose: () => void }) {
  return <div className="fixed inset-0 z-[70] bg-black/45 p-3 md:hidden" onClick={onClose}><div className="motion-slide absolute inset-x-3 bottom-24 rounded-[22px] border border-line bg-surface p-4 shadow-card" onClick={(e) => e.stopPropagation()}><div className="flex items-center justify-between"><div><p className="text-xs text-muted">More</p><h2 className="text-lg font-bold text-ink">Tools and account</h2></div><button onClick={onClose} className="rounded-card border border-line px-3 py-2 text-sm text-muted">Close</button></div><div className="mt-4 grid grid-cols-2 gap-2">{items.map((item) => <MoreLink key={`${item.label}-${item.href}`} item={item} active={isActive(pathname, item.href)} onClose={onClose} />)}</div></div></div>;
}

function AppNavLink({ item, active }: { item: { label: string; href: string; icon: () => React.ReactNode }; active: boolean }) { const Icon = item.icon; return <Link href={item.href} className={`motion-press flex h-11 items-center gap-3 rounded-card px-3 text-sm ${active ? "bg-brand text-white" : "text-muted hover:bg-surface-2 hover:text-ink"}`}><Icon /><span>{item.label}</span></Link>; }
function MobileNavLink({ item, active }: { item: (typeof primaryTabs)[number]; active: boolean }) { const Icon = item.icon; return <Link href={item.href} className={`motion-press flex min-h-[64px] flex-col items-center justify-center gap-1 rounded-card px-2 py-2 text-[11px] ${"featured" in item && item.featured ? active ? "bg-brand text-white shadow-card" : "bg-brand/12 text-brand" : active ? "bg-brand text-white" : "text-muted"}`}><Icon /><span>{item.label}</span></Link>; }
function MoreLink({ item, active, onClose }: { item: { label: string; href: string; icon: () => React.ReactNode }; active: boolean; onClose: () => void }) { const Icon = item.icon; return <Link href={item.href} onClick={onClose} className={`motion-press flex items-center gap-3 rounded-card border px-3 py-3 text-sm ${active ? "border-brand bg-brand text-white" : "border-line bg-surface-2 text-ink"}`}><Icon /><span>{item.label}</span></Link>; }
...
function HomeIcon() { return iconPath(<path d="M4 10.5 12 4l8 6.5V20a1 1 0 0 1-1 1h-5v-6h-4v6H5a1 1 0 0 1-1-1v-9.5Z" stroke="currentColor" strokeLinejoin="round" strokeWidth="2" />); }
...
function BellIcon() { return iconPath(<path d="M18 9a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9ZM10 21h4" stroke="currentColor" strokeLinecap="round" strokeWidth="2" />); }
