    ...
    <main className="min-h-screen bg-bg px-4 py-5 text-ink">
      ...
      <div className="mx-auto mt-8 max-w-3xl rounded-card border border-line bg-surface p-5 shadow-card sm:p-6">
        ...
        <p className="mt-3 text-sm leading-6 text-muted">
          These settings drive mock watchlists, alerts, Everything Mode, and Deal Check thresholds.
        </p>
        ...
        <form className="mt-6 grid gap-5" onSubmit={handleSubmit}>
          ...
          <label className="flex items-center justify-between gap-3 rounded-card border border-line bg-surface-2 p-3">
            <span>
              <span className="block text-sm font-bold text-ink">Everything Mode enabled?</span>
              <span className="block text-sm text-muted">Snagd can watch anything profitable based on your thresholds.</span>
            </span>
            <input
              type="checkbox"
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="ZIP code or city">
              <input className="field" value={profile.location} onChange={(event) => setProfile({ ...profile, location: event.target.value })} />
            </Field>
            <Field label="Alert preference">
              <select className="field" value={profile.alertPreference} onChange={(event) => setProfile({ ...profile, alertPreference: event.target.value })}>
                {["Standard in-app", "Fast in-app + email", "Priority in-app + email", "SMS when available"].map((item) => (
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <button
              ...
              type="button"
              onClick={() => finish(defaultOnboardingProfile)}
            >
              Use demo settings
            </button>
          </div>
