  ...
  return (
    <div className="mx-auto max-w-[430px] md:max-w-shell">
      <div className="mb-4 flex items-end justify-between gap-3">
        <div><h1 className="text-2xl font-bold text-ink">Live deal feed</h1><p className="mt-1 text-sm text-muted">Photo-first flips sorted by margin, score, distance, and timing.</p></div>
      ...
      <div className="-mx-4 mb-4 overflow-x-auto px-4 no-scrollbar">
        <div className="flex gap-2">
          {chips.map((chip) => <button key={chip} type="button" onClick={() => setActive(chip)} className={`motion-press whitespace-nowrap rounded-full border px-3 py-2 text-sm font-bold ${active === chip ? "border-brand bg-brand text-white" : "border-line bg-surface text-muted"}`}>{chip}</button>)}
        </div>
      </div>
