...
const chips = ["Smart Picks", "Everything", "Vehicles", "Furniture", "Tools", "Electronics", "Free", "Sneakers", "Appliances", "Watchlists"];

export function DealFeedClient({ deals }: { deals: Deal[] }) {
  const [active, setActive] = useState("Smart Picks");
  ...
  const filtered = useMemo(() => {
    if (active === "Smart Picks") return deals.filter((deal) => deal.score >= 76 || deal.recommendation === "BUY");
  ...
  return (
    <div className="mx-auto max-w-[430px] md:max-w-shell">
      ...
      {!filtered.length && <div className="rounded-[18px] border border-line bg-surface p-5 text-sm text-muted">No deals match this chip yet. Try Everything or Smart Picks.</div>}
    </div>
  );
}
