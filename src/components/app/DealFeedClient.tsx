...
export function DealFeedClient() {
  ...
  useEffect(() => {
    ...
    const loadSaved = () => { try { setSavedIds(JSON.parse(window.localStorage.getItem("snagd-saved-deal-ids") || "[]")); } catch { setSavedIds([]); } };
    loadSaved();
    window.addEventListener("snagd:saved-deals-changed", loadSaved);
    if (window.localStorage.getItem("snagd-automotive-mode") === "true") setFilters((current) => ({ ...current, category: "Vehicles" }));
    return () => {
      window.removeEventListener("snagd:saved-deals-changed", loadSaved);
    };
  }, []);

  useEffect(() => {
    let active = true;
    async function load(refreshDue = false) {
      try {
  ...
  async function saveConfiguration(kind: "search" | "watchlist") {
    try {
    ...
    <div className="mx-auto max-w-[430px] md:max-w-shell">
      <section className="mb-4 rounded-[22px] border border-brand/30 bg-surface p-4 shadow-card">
        ...
        <div className="mt-3 grid grid-cols-[1fr_110px] gap-2 sm:grid-cols-[1fr_150px_auto]">
          <input aria-label="ZIP code" inputMode="numeric" maxLength={10} value={zip} onChange={(event) => setZip(event.target.value)} placeholder="ZIP: 08102" className="deal-feed-field" />
          <select aria-label="Radius" value={filters.radius} onChange={(event) => update("radius", Number(event.target.value))} className="deal-feed-field">{[5, 10, 25, 50, 100].map((radius) => <option key={radius} value={radius}>{radius} miles</option>)}</select>
      ...
      <details className="mb-4 rounded-[20px] border border-line bg-surface shadow-soft">
        ...
        <div className="grid gap-4 border-t border-line p-4">
          <div className="grid gap-3 sm:grid-cols-3"><Field label="Keyword"><input className="deal-feed-field" value={filters.keyword} onChange={(e) => update("keyword", e.target.value)} placeholder="Camry, tools..." /></Field><Field label="Include keywords"><input className="deal-feed-field" value={filters.includeKeywords} onChange={(e) => update("includeKeywords", e.target.value)} placeholder="clean title" /></Field><Field label="Exclude keywords"><input className="deal-feed-field" value={filters.excludeKeywords} onChange={(e) => update("excludeKeywords", e.target.value)} placeholder="broken, parts" /></Field></div>
...
function NumberField({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) { return <Field label={label}><input className="deal-feed-field" type="number" min="0" inputMode="numeric" value={value || ""} onChange={(e) => onChange(Number(e.target.value))} /></Field>; }
function SelectField({ label, value, options, onChange }: { label: string; value: string; options: readonly string[]; onChange: (value: string) => void }) { return <Field label={label}><select className="deal-feed-field" value={value} onChange={(e) => onChange(e.target.value)}>{options.map((option) => <option key={option}>{option}</option>)}</select></Field>; }
