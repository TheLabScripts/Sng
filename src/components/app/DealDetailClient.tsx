...
export function DealDetailClient({ deal }: { deal: Deal }) {
  ...
  return (
    <div className="mx-auto max-w-[430px] pb-8">
      ...
      <AppCard className="mt-5"><div className="flex items-center justify-between gap-3"><div><p className="text-sm text-muted">Value meter</p><h2 className="mt-1 text-lg font-bold text-ink">Estimated value: ${resaleMid}</h2></div><span className="rounded-full bg-brand/10 px-3 py-1 text-sm font-bold text-brand">{valueLabel}</span></div><div className="relative mt-5 h-3 rounded-full value-meter"><span className="absolute top-1/2 h-8 w-1 -translate-y-1/2 rounded-full bg-info shadow-card" style={{ left: `${marker}%` }} /></div><div className="mt-2 flex justify-between text-[11px] text-muted"><span>Overpriced</span><span>Fair</span><span>Under market</span></div><p className="mt-3 text-sm text-muted">Current listing is estimated <span className="font-bold text-profit">{deal.underMarketPercent}% under market</span> based on {deal.similarSalesCount} seeded similar sales.</p></AppCard>

      <AppCard className="mt-4 bg-gradient-to-r from-brand/14 to-surface"><div className="flex items-center justify-between gap-4"><div><p className="text-sm text-muted">Snagd Score</p><p className="font-mono text-4xl font-bold text-brand">{deal.score}</p><p className="mt-1 text-sm text-muted">Based on {deal.similarSalesCount} similar sales / Demand: {deal.demand} / Competition: {deal.competition} / Confidence: {deal.confidence}</p></div><div className="rounded-[14px] bg-brand px-5 py-4 text-center text-white"><p className="text-xl font-bold">{deal.recommendation}</p><p className="text-xs">action rating</p></div></div><div className="mt-4 flex flex-wrap gap-2">{deal.reasonTags.slice(0, 5).map((tag) => <span key={tag} className={`rounded-full border px-2.5 py-1 text-[11px] ${tagClass(tag)}`}>{tag}</span>)}</div></AppCard>
...
function SimilarSalesSheet({ dealTitle, sales, onClose }: { dealTitle: string; sales: SimilarSale[]; onClose: () => void }) { return <div className="fixed inset-0 z-[80] grid place-items-end bg-black/50 p-3 sm:place-items-center" role="dialog" aria-modal="true"><div className="motion-slide max-h-[86vh] w-full max-w-lg overflow-auto rounded-[22px] border border-line bg-surface p-4 shadow-card"><div className="flex items-start justify-between gap-3"><div><p className="text-sm text-muted">Similar Sales</p><h3 className="text-xl font-bold text-ink">{dealTitle}</h3></div><button onClick={onClose} className="rounded-card border border-line px-3 py-2 text-sm text-muted">Close</button></div><div className="mt-4 grid gap-3">{sales.length ? sales.map((sale) => <SimilarSaleRow key={sale.id} sale={sale} />) : <p className="rounded-card border border-line bg-surface-2 p-3 text-sm text-muted">No comps are attached to this listing yet.</p>}</div></div></div>; }
