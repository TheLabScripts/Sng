    ...
    <article className="motion-card overflow-hidden rounded-[18px] border border-line bg-surface shadow-card">
      <div className="flex gap-3 p-3">
        ...
        <div className="min-w-0 flex-1 py-1">
          <div className="flex items-start justify-between gap-2">
            ...
            <button type="button" onClick={toggleSaved} className={`motion-press grid h-9 w-9 place-items-center rounded-full border ${saved ? "border-brand bg-brand text-white save-pop" : "border-line bg-surface-2 text-muted"}`} aria-label={saved ? "Unsave deal" : "Save deal"}><HeartIcon filled={saved} /></button>
          </div>
      ...
      <div className="mx-3 rounded-[14px] border border-line bg-surface-2 p-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="text-right"><p className="text-sm font-bold text-profit">Under market by {deal.underMarketPercent}%</p><p className="text-[11px] text-muted">{deal.similarSalesCount} sales / {deal.demand} demand / {deal.competition} comp.</p></div>
        </div>
      </div>

      {!compact && <div className="flex flex-wrap gap-2 p-3 pt-2">{deal.reasonTags.slice(0, 3).map((tag) => <span key={tag} className={`rounded-full border px-2.5 py-1 text-[11px] ${tag.toLowerCase().includes("risk") ? "border-pass/25 bg-pass/10 text-pass" : "border-profit/25 bg-profit/10 text-profit"}`}>{tag}</span>)}</div>}
...
function Metric({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) { return <div><p className="text-[10px] text-muted">{label}</p><p className={`mt-0.5 font-mono text-sm font-bold tnum ${accent ? "text-profit" : "text-ink"}`}>{value}</p></div>; }
