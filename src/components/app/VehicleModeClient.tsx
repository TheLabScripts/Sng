...
const sampleVin = "1HGCV1F3XMA123456";

export function VehicleModeClient() {
  const [vin, setVin] = useState(sampleVin);
  ...
  return (
    <div className="mx-auto grid max-w-[430px] gap-4 md:max-w-shell lg:grid-cols-[0.9fr_1.1fr]">
      <AppCard className="content-start">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3"><ScannerTile label="Door-jamb barcode" /><ScannerTile label="VIN OCR" /></div>
        <AppCard className="mt-4"><h2 className="text-lg font-bold text-ink">Plan access</h2><div className="mt-3 grid gap-2 text-sm text-muted"><p>Starter: manual listing analyzer and saved vehicle notes</p><p>Pro: basic VIN lookup, recall/risk review, and comps</p><p>Power: Vehicle/VIN Intelligence, more VIN checks, and bulk similar sales</p></div><div className="mt-4 grid gap-2 text-sm"><Pack label="10 VIN Checks" price="$5" /><Pack label="50 VIN Checks" price="$19" /><Pack label="200 VIN Checks" price="$59" /></div></AppCard>
      </AppCard>
      ...
      <AppCard className="overflow-hidden rounded-[24px] p-0">
        ...
        <div className="mx-5 mt-5 rounded-[18px] border border-line bg-surface-2 p-4"><div className="flex items-center justify-between gap-4"><div className="flex items-center gap-3"><span className="grid h-12 w-12 place-items-center rounded-[14px] border border-brand/45 text-xl font-bold text-brand">88</span><div><p className="text-sm font-bold text-ink">Buy/Maybe/Pass</p><p className="text-xs text-muted">{vehicle.recommendation} / {riskSummary.riskLevel} risk</p></div></div><div className="text-right"><p className="text-xs text-muted">Suggested offer</p><p className="text-xl font-bold text-profit">{currency(vehicle.suggestedMaxOffer)}</p></div></div></div>
...
function ReportSection({ title, items }: { title: string; items: string[] }) { return <section><h3 className="font-bold text-ink">{title}</h3><ul className="mt-3 grid gap-2 text-sm text-muted">{items.map((item) => <li key={item} className="flex gap-2"><span className="text-brand">OK</span><span>{item}</span></li>)}</ul></section>; }
