    ...
    <div className="mx-auto grid max-w-shell gap-4 lg:grid-cols-[0.85fr_1.15fr]">
      ...
      <AppCard>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-sm text-muted">Creator code dashboard placeholder</p>
            <h2 className="mt-1 text-2xl font-bold text-ink">{mockCreatorStats.creatorCode}</h2>
          </div>
          <Link href="/creators/" className="inline-flex h-10 items-center rounded-card border border-line px-3 text-sm text-muted">
            Creator page
          </Link>
        </div>
...
function CreatorMetric({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) {
  return (
    <div>
