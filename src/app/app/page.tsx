  ...
  return (
    <div className="mx-auto grid max-w-[430px] gap-5 md:max-w-shell">
      ...
      <section className="grid gap-3 md:grid-cols-3">
        ...
        <AppCard><div className="flex items-center justify-between"><h2 className="text-lg font-bold text-ink">Saved Deals</h2><Link href="/app/saved/" className="text-sm font-bold text-brand">Open</Link></div><p className="mt-3 font-mono text-3xl font-bold text-brand">{savedDealSeeds.length}</p><div className="mt-3 grid gap-2 text-sm text-muted">{savedDealSeeds.slice(0, 2).map((deal) => <p key={deal.id}>{deal.itemName} / {deal.estimatedProfit}</p>)}</div></AppCard>
      </section>

      <AppCard><div className="flex items-start justify-between gap-4"><div><h2 className="text-lg font-bold text-ink">Deal Checks Remaining</h2><p className="mt-1 text-sm text-muted">{mockUsage.dealChecksLimit - mockUsage.dealChecksUsed} checks left before {mockUsage.billingCycleEnds}</p></div><p className="font-mono text-2xl font-bold text-brand">{mockUsage.dealChecksLimit - mockUsage.dealChecksUsed}</p></div><div className="mt-4"><UsageMeter used={mockUsage.dealChecksUsed} limit={mockUsage.dealChecksLimit} label="Deal Checks used" /></div></AppCard>
