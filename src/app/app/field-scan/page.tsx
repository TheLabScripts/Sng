    ...
    <div className="mx-auto grid max-w-[430px] gap-4 md:max-w-shell lg:grid-cols-[0.85fr_1.15fr]">
      ...
      <div className="grid content-start gap-4">
        {resultVisible ? <AppCard className="brand-ring">
          <div className="flex items-start justify-between gap-3"><div><p className="text-sm text-muted">Item detected</p><h2 className="mt-1 text-2xl font-bold text-ink">{fieldScanResult.itemDetected}</h2><p className="mt-1 text-sm text-muted">{fieldScanResult.confidence} confidence / {fieldScanResult.suggestedCategory}</p></div><RecommendationBadge value={fieldScanResult.recommendation} /></div>
